...
;------------------------------------------------------------
;	preemptive.c:56: ThreadID ThreadCreate(FunctionPtr fp) __critical{
;	-----------------------------------------
...
;------------------------------------------------------------
;	preemptive.c:180: void myTimer0Handler(){
;	-----------------------------------------
;	 function myTimer0Handler
;	-----------------------------------------
_myTimer0Handler:
;	preemptive.c:181: EA = 0;
;	assignBit
	clr	_EA
;	preemptive.c:182: SAVESTATE;
	PUSH ACC 
	PUSH B 
	PUSH DPL 
	PUSH DPH 
...
;	preemptive.c:200: __endasm;
	...
	MOV	A, R7
	PUSH	ACC
;	preemptive.c:204: if(current_id==3) current_id=0;
	mov	a,#0x03
	cjne	a,_current_id,00102$
	mov	_current_id,#0x00
	sjmp	00103$
00102$:
;	preemptive.c:205: else current_id++;
	mov	a,_current_id
	inc	a
	mov	_current_id,a
00103$:
;	preemptive.c:207: if (bitmap == 1)
	mov	a,#0x01
	cjne	a,_bitmap,00116$
;	preemptive.c:209: if (current_id > 0)
	mov	a,_current_id
	jz	00117$
;	preemptive.c:210: current_id = 0;
	mov	_current_id,#0x00
	sjmp	00117$
00116$:
;	preemptive.c:212: else if (bitmap == 3)
	mov	a,#0x03
	cjne	a,_bitmap,00113$
;	preemptive.c:214: if (current_id > 1)
	mov	a,_current_id
	add	a,#0xff - 0x01
	jnc	00117$
;	preemptive.c:215: current_id = 0;
	mov	_current_id,#0x00
	sjmp	00117$
00113$:
;	preemptive.c:217: else if (bitmap == 7)
	mov	a,#0x07
	cjne	a,_bitmap,00117$
;	preemptive.c:219: if (current_id > 2)
	mov	a,_current_id
	add	a,#0xff - 0x02
	jnc	00117$
;	preemptive.c:220: current_id = 0;
	mov	_current_id,#0x00
00117$:
...
;------------------------------------------------------------
;	preemptive.c:255: void ThreadExit(void) __critical{
;	-----------------------------------------
;	 function ThreadExit
;	-----------------------------------------
_ThreadExit:
